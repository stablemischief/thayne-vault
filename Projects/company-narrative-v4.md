...
Two regulatory dynamics are shaping this moment in ways that favor builders who move now.

AgFin's initial commercial target is independent agricultural lenders — community banks, private crop lenders, and farm lending cooperatives that are not Farm Credit System institutions and therefore not directly regulated by the Farm Credit Administration. This is a deliberate market entry strategy. Shorter sales cycles, fewer compliance procurement gates, and faster pilots. These lenders face the same document volume problem as Farm Credit associations and have even fewer technology solutions available to them. They are the right first customers.

The Farm Credit Administration's Cyber Risk Management rule, effective January 2025, governs Farm Credit System institutions directly.[^8] While the rule does not reach independent lenders, it does require FCS institutions to actively manage the cyber risk of their technology vendors — meaning any vendor seeking to sell into the Farm Credit System must be prepared to undergo due diligence review, provide evidence of security controls, and demonstrate operational resilience. Stable Mischief is building toward that standard now. We are currently pursuing SOC 2 Type 1 certification, which provides a point-in-time attestation that our security controls are properly designed and implemented. SOC 2 Type 1 positions us for the SOC 2 Type 2 audit — a 6–12 month observation period that produces the continuous compliance evidence Farm Credit institutions will require before approving a vendor. Vendors entering the space today who have not begun this work face a minimum 12–18 month runway before they can have that conversation with a Farm Credit buyer. We have that infrastructure in production and the certification process underway.
...
Stable Mischief has identified three patentable innovations within this architecture, with provisional patent filings in progress:

1. **Dual-Model Consensus Extraction with Selective Arbitration** — Two AI models from independent model families extract data in parallel. A domain-aware comparison layer identifies genuine disagreements versus cosmetic differences. Only disputed fields — typically 5–20% of the total — are escalated to a third model for arbitration. Prior art in document extraction uses single-model approaches or simple ensemble voting. The selective arbitration mechanism, with domain-specific normalization tuned to agricultural document conventions, represents a novel approach to extraction accuracy in regulated document processing.

2. **Confidence-Weighted Extraction with Mandatory Human Triage Gating** — A multi-factor confidence score (format validation, completeness, extraction metadata) is computed per field and used to enforce workflow gates. Fields below threshold require human review before the system permits certification. This is not a dashboard metric — it is a hard architectural constraint that prevents low-confidence data from reaching downstream calculations.

3. **Immutable Audit Trail with Cryptographic Hash Chain for Lending Workflows** — Three independent tamper-evidence mechanisms (database triggers blocking modification, no-modification API surface, SHA-256 hash chain linking sequential entries) combined with automatic PII encryption before audit storage and mandatory justification codes for every edit.
...
AgFin was built in under 120 days, by a team of fewer than five people. It is one of five AI products the Stable Mischief team has built in the past 12 months — a track record of velocity that the methodology was designed to sustain. 
...
Agriculture doesn't have a data problem. It has a visibility problem.

No one has a clear, ground-level picture of where agricultural organizations are losing money, wasting time, and operating without adequate tools — because no one has been inside enough of them, at the process level, to see the patterns. That is exactly what aiBA produces. Every engagement is a structured map of how a real agricultural organization actually works. Anonymized and aggregated across dozens, then hundreds of clients, those maps reveal the problems the industry hasn't yet figured out how to articulate.

AgFin is the first product. It is not the thesis.

The thesis is that Stable Mischief has built a system for identifying the next problem before the market can name it, and a methodology for building the solution in weeks rather than years. The current portfolio — AgFin, Kowerk, aiBA — is the first proof of that system working. The products that follow will be validated by data that no competitor has access to, built by a team that has already demonstrated it can ship production-grade platforms at a fraction of conventional cost and time.

Investors who come in at this round are not buying a document intelligence platform. They are buying into the machine that builds what agriculture needs next. The barn fills with every product shipped and every aiBA engagement completed. This round funds the first contents. The structure is already standing.
...
**AgFin** generates revenue on a per-application basis at $100 per application — lenders pay for each loan application processed through the platform. This outcome-based model scales directly with their business volume and mirrors how comparable platforms like Ocrolus price their services. As AgFin deepens into a full credit solution, the model matures to annual SaaS licensing:

- Community banks (<500 loans/year): ~$50K–$100K ARR
- Regional banks (500–3,000 loans/year): ~$120K–$300K ARR
- Farm Credit enterprises: $250K–$750K+ ARR

At 24 months, the model expands to a third stage: an agricultural lending marketplace where farmers upload documents once and receive competing lender offers. Origination-based revenue at a fundamentally different scale than the lender-facing tool alone.
...
**Use of funds — 24-month allocation:**

| Category | Allocation | At $7.5M | At $10M |
|---|---|---|---|
| **Team** — competitive salaries for founding team + incoming full-time developers, enterprise sales specialist, customer success | ~40% | ~$3.0M | ~$4.0M |
| **AI & Infrastructure** — model API costs, cloud compute, security tooling, SaaS subscriptions | ~17% | ~$1.3M | ~$1.7M |
| **Sales & Market Development** — independent lender pilot programs, Farm Credit enterprise sales infrastructure, industry conference presence | ~15% | ~$1.1M | ~$1.5M |
| **Compliance & Legal** — SOC 2 Type 1 (underway) and Type 2 audit completion[^8], non-provisional patent filings, ongoing corporate counsel | ~8% | ~$600K | ~$800K |
| **Working Capital** — operations, travel for client and investor relationships, tools | ~10% | ~$750K | ~$1.0M |

The Team bucket funds four to six people at market rate for 24 months: founding team (James Whitfield, Mitch Dornich), incoming full-time (Tristan Cuevas as Head of Development, Sean Buck as CTO), plus two strategic hires in enterprise ag lending sales and customer success. The AI & Infrastructure bucket reflects the actual cost structure of running a multi-agent AI platform at scale — a line item most ag-tech pitches omit entirely but one that Stable Mischief has modeled from production data.
...
[^8]: **VERIFIED.** FCA Cyber Risk Management Rule, effective January 1, 2025. Published Federal Register December 11, 2023. Rule applies directly to Farm Credit System institutions (banks, associations, Farmer Mac) — not to independent lenders or technology vendors. However, the rule requires FCS institutions to implement vendor/third-party cyber risk management programs, creating downstream due diligence requirements for any technology vendor seeking to serve the Farm Credit System. SOC 2 Type 2 is the industry-standard attestation used to satisfy those requirements. Source: Federal Register 88 FR 85728, https://www.federalregister.gov/documents/2023/12/11/2023-27102/cyber-risk-management
...
*Draft 4 — CONFIDENTIAL. For review only. Not for external distribution.*
*Key changes from V3: (1) Clarified FCA rule scope — applies to FCS institutions, not independent lenders or vendors; (2) Added explicit market entry strategy — independent crop lenders first, FCS institutions second; (3) Added SOC 2 Type 1 → Type 2 certification path; (4) Corrected footnote [^8] to accurately describe rule applicability and vendor due diligence dynamic.*
*Additional updates (March 24): (5) Patent section expanded — three patentable innovations listed with descriptions, filings in progress (not yet filed); (6) AgFin pricing added — $100/application with ARR tier ranges; (7) Three-stage revenue model added (per-app → SaaS → marketplace); (8) Use of funds expanded to full five-bucket table with dollar allocations at $7.5M and $10M raise; (9) "The Long Game" section added — standalone flywheel/machine narrative before The Unique Insight; (10) AgFin traction line added — 5 AI products built in 12 months.*
